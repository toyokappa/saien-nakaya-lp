...
resource "aws_cloudfront_distribution" "lp_www" {
  wait_for_deployment = false
  enabled             = true
  is_ipv6_enabled     = true
  comment             = "${local.app_name}-${terraform.workspace}-lp-www"
  default_root_object = ""
  aliases             = ["www.${local.lp_domain}"]

  origin {
    domain_name = aws_s3_bucket_website_configuration.lp_www.website_endpoint
    origin_id   = aws_s3_bucket_website_configuration.lp_www.id
  ...
  default_cache_behavior {
    allowed_methods  = ["HEAD", "GET", "OPTIONS"]
    cached_methods   = ["HEAD", "GET"]
    target_origin_id = aws_s3_bucket_website_configuration.lp_www.id
    compress         = true
